---
description: Pre-release verification checklist for stream-sorting
---
...
1. Check that each public component's `src/<name>.js` and `src/<name>.d.ts` are in sync (the exported function and option shapes match).
...
7. Verify `package.json`:
   ...
   - `dependencies` contains only `stream-chain` and `stream-join`.
