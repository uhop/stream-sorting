...
- **`matching` / `unmatched`** — filters emitting rows of the primary stream whose key is present / absent in the other (the former semi/anti joins; share the merge walk with the set-ops).
- **`aggregate`** — fold child streams under a master (or a key-derived group) by key; covers SQL-style group/aggregate, with multi-level nesting done by composition (join-enrich → re-sort → aggregate).
- **`union` / `intersection` / `difference`** — set operations on sorted streams by value (`lessFn`).
- **`merge`** — k-way sorted merge without dedup (was `mergeSorted`); may subsume the copy in `stream-join/utils/merge-sorted` (open decision; see vault `projects/stream-sorting/queue.md`).
